...
- `core.test.js` — geometry, row planning, cutting-stock packing, the scale
...
- `report.test.js` — the printed manual. Asserts the document is greyscale only
  ...
  sheet.
- `editor.test.js` — pushes real pointer events through the tracing canvas. The
...
- `spa.test.js` — drives the *built* `dist/index.html` under jsdom with real DOM
