...
import { deriveRooms } from '../src/core/derive.js';
...
function fixture() {
  ...
  const pins = [{ id: 1, roomId: 1, shapeId: 1, edge: 'top', metres: 6 }];
...
test('manual builds and covers all three phases', () => {
  ...
  const html = buildReport(rooms, {}, pxPerM);
...
test('no key plan without a scale, and no crash either', () => {
  const rooms = [{ id: 1, name: 'A', code: 'A', dir: 'x', skip: false, shapes: [], add: [{ x: 0, y: 0, w: 3, h: 3 }], sub: [] }];
  const html = buildReport(rooms, {}, null);
  assert.ok(html);
  assert.doesNotMatch(html, /Key plan/);
});
